...
atomic<bool> solutionTrouve(false);
vector<pair<int,int>> solutionChemin;
mutex solutionMutex;
...
int main() {
    ...
    for (int numero = 0; numero < labyrinthes.size(); ++numero) {
        ...
        // préparation du plusieur threads
        solutionTrouve.store(false);
        solutionChemin.clear();

        // Backtracking
        vector<vector<bool>> isCaseVisitee(nbrLigneTotal, vector<bool>(nbrColonnesTotal, false));

        //vector<pair<int,int>> cheminParcouru; // (x,y)
        bool departObjet = labyrinthe[caseDepart_x][caseDepart_y].estObjetAPrendre;
        ...
        auto sniffer = [&](int dir) {



            int nouvelleCoodonneeCase_x = caseDepart_x + directions[dir].first;
            int nouvelleCoodonneeCase_y = caseDepart_y + directions[dir].second;


                // copie de l'étatS
                auto visiteeEtat = isCaseVisitee;
                vector<pair<int,int>> cheminParcouru;

                // marquer le départ
            visiteeEtat[caseDepart_x][caseDepart_y] = true;

            cheminParcouru.push_back({caseDepart_x, caseDepart_y});

            bool trouve = resoudreLabyrinthe(labyrinthe, visiteeEtat, nouvelleCoodonneeCase_x, nouvelleCoodonneeCase_y, caseArrivee_x, caseArrivee_y, cheminParcouru, departObjet, fautIlRamasserObjet);




            if (trouve) { // on bloque poru la reponse et changer le booleen mutex à true.
                lock_guard<mutex> lock(solutionMutex);
                if (!solutionTrouve.load()) {
                    solutionChemin = cheminParcouru;
                    solutionTrouve.store(true);
                }
            }
        ...
        for (int dir = 0; dir < 4; ++dir) {
            threads.push_back(thread(sniffer, dir));
        }
...
// On récupère et on marque le chemin trouvé
        for (int i = 0; i < solutionChemin.size(); ++i) {
            // pas de structured binding, on récupère manuellement
            int x = solutionChemin[i].first;
            int y = solutionChemin[i].second;
        ...
        for (int i = 0; i < labyrintheEnCours.size(); ++i) {
            cout << "  ";
            // on travaille sur une copie de la ligne pour simplifier
            string ligne = labyrintheEnCours[i];
    ...
    cout << "\nTemps total pour résoudre tous les labyrinthes : " << duree_ms << " ms\n";
